...
def to_dataframe(data):
    ...
    df = pd.concat([df.drop(['audio_features'], axis=1), df['audio_features'].apply(pd.Series)], axis=1)
    df.drop(['id', 'uri', 'analysis_url', 'time_signature', 'type', 'track_href'], axis=1, inplace=True)
    return df
